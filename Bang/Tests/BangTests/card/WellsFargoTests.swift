...
final class WellsFargoTests: CardTests {
    ...
    func test_Draw3Cards_IfPlayingWellsFargo() throws {
        ...
        assertIsSuccess(events[1], equalTo: Draw(player: .id("p1")))
        assertIsSuccess(events[2], equalTo: Draw(player: .id("p1")))
        assertIsSuccess(events[3], equalTo: Draw(player: .id("p1")))
