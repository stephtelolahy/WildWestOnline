...
extension Card.ActionName {
    func reduce(_ action: GameFeature.Action, state: GameFeature.State, dependencies: GameFeature.Dependencies) throws(GameFeature.Error) -> GameFeature.State {
...
private extension Card.ActionName {
    ...
    struct ApplyModifier: Reducer {
        func reduce(_ action: GameFeature.Action, state: GameFeature.State, dependencies: GameFeature.Dependencies) throws(GameFeature.Error) -> GameFeature.State {
            ...
            guard let handler = dependencies.registry.handler(for: modifier) else { fatalError("No handler for modifier: \(modifier)")}
